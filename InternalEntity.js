...
var ultimateParentId = entityData[0]["internalEntity"].ultimateParent.id;

var getHeirarchy = data.filter((internalEntities) => {
    var returnValue = false;
    if(internalEntities["internalEntity"]["ultimateParent"]){
        if(internalEntities["internalEntity"]["ultimateParent"].id == ultimateParentId) {
            returnValue = true;
        }
    } 
    if(!returnValue) {
          if(internalEntities["internalEntity"].id == ultimateParentId){
              returnValue = true;
          } 
    }
...
var structuredData = getHeirarchy.map(internalEntities => {
  return {
    id: internalEntities["internalEntity"].id,
    parentId: internalEntities["internalEntity"]["parent"] ? internalEntities["internalEntity"]["parent"].id: null,
    entityName: internalEntities["internalEntity"]["entityName"],
    data: {
...
var finalData = structuredData.filter((entity, index, originalArray) => {

  var childData = [];

  for(let i = 0; i< originalArray.length; i++) {
        if(originalArray[i].parentId == entity.id) {
          childData.push(originalArray[i])
        }
  }
